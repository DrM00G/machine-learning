...
class DataFrame:
    ...
    def rename_columns(self,new_columns):
        ...
        for i  in range(len(new_columns)):
            new_dict[new_columns[i]] = self.ordered_dict[self.columns[i]]
        # print(new_dict)
    ...
    def from_csv(cls, file_path, header = True):
        ...
        # print(lines[0][0].split('"')[3])
        # return_list = [person[0].split(', ') for person in lines if lines.index(person)!=0]
